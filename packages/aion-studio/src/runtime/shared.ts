...
import type { AgentTraceOptions, JsonObject, JsonValue, Message } from "@deltakit/aion";
...
const baseUnsupportedCapabilities = [
  "memory",
  "knowledge",
  "metrics",
  "evals",
...
export function resolveStores(options: StudioRuntimeOptions): ResolvedStores {
  const defaultPath =
    ...
    join(process.cwd(), ".aion-studio", `${safeFileName(runnerId(options))}.sqlite`);
...
export function capabilityConfig(
  ...
  if (agents.some((agent) => agent.agent.hook?.onToolCall !== undefined)) {
...
export function safeFileName(value: string): string {
  return value.replace(/[^a-zA-Z0-9._-]+/g, "_") || "aion-studio";
}
