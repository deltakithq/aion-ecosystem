...
export function resolveStudioUiOptions(
  ...
  return {
    ...
    title: options.title ?? "Aion Studio",
...
function renderLegacyStudioUiShell(props: {
  ...
  return [
    ...
    `<title>${title}</title>`,
    '<script>(()=>{try{if(localStorage.getItem("aion-studio-theme")==="light"){document.documentElement.classList.remove("dark")}}catch{}})();</script>',
    `<link rel="icon" type="image/png" href="${escapeHtml(props.compatUiPath)}/assets/aion.png">`,
    `<link rel="stylesheet" href="${escapeHtml(props.stylesheet)}">`,
    "</head>",
    "<body>",
    `<div id="aion-ui" data-ui-path="${escapeHtml(props.uiPath)}" data-ui-compat-path="${escapeHtml(props.compatUiPath)}">`,
    '<main class="shell-loading">',
    "<div>",
    `<img src="${escapeHtml(props.compatUiPath)}/assets/aion.png" alt="" width="32" height="32">`,
    '<p class="eyebrow">Aion</p>',
    `<h1>${title}</h1>`,
    "</div>",
    "</main>",
    "</div>",
...
async function readBundledUiIndex(): Promise<string> {
  try {
    ...
    return readFile(fileURLToPath(new URL("../../dist/ui/index.html", import.meta.url)), "utf8");
  }
}
