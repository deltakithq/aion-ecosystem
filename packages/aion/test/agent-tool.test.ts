...
describe("Agent.asTool", () => {
  ...
  it("does not expose provider-level agent builders", () => {
    const client = new OpenRouterClient({ apiKey: "test" });

    expect("agent" in client).toBe(false);
    // @ts-expect-error provider clients no longer construct agents
    expect(client.agent).toBeUndefined();
    expect("extractor" in client).toBe(false);
    // @ts-expect-error provider clients no longer construct extractors
    expect(client.extractor).toBeUndefined();
  });
